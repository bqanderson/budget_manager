  ...
  <div class="l-budget-creation">
    <v-layout row wrap>
      ...
      <v-layout row wrap v-for="item in budget.items" class="l-budget-item" :key="item.id">
        <v-flex xs12 md1>
          <v-btn block dark color="red lighten-1" @click-native="removeItem(iten)">Remove</v-btn>
        </v-flex>
      ...
      <v-flex xs12 md2 offset-md10>
        <v-btn block color="md-add-btn light-blue lighten-1" @click-native="addItem()">Add Item</v-btn>
      </v-flex>

      <v-flex xs12 md2 offset-md10>
        <span class="md-budget-item-total white--text">TOTAL $ {{ item.total_price }}</span>
      </v-flex>

      <v-flex xs12 md2 offset-md10>
        <v-btn block color="md-add-btn green lighten-1" @click-native="saveBudget(budget)">Save</v-btn>
      </v-flex>
    </v-layout>
  </div>
</template>
